...
export default function CreateTask() {
    ...
    const [userGroups, setUserGroups] = useState<[{name: string, id: string}]>();
    ...
    useEffect(() => {
        ...
        if (elemDate != null) {
            let temp = ""
            if (dateElems.getMonth()+1 < 10)
                temp = '0'
            const dateL = dateElems.getFullYear()+"-"+temp+(dateElems.getMonth()+1)+"-"+dateElems.getDate();
    ...
    function handleChangeTime (newTime: string, dateCheck: string) {
        ...
        const lDate = new Date(dateCheck)
        lDate.setHours(Number(newTime.substring(0,2)), Number(newTime.substring(3,5)))
        if (lDate < nowDate) {
            setTime(nowDate.toTimeString().substring(0,5))
            return
        }
        setTime(newTime)
    }
    function handleChangeDate (newDate: string) {
        const nowDate = new Date()
        const newDateL = new Date(newDate)
        let dateL = newDate
        if (newDateL < nowDate) {
            let temp = ''
            if (nowDate.getMonth()+1 < 10)
                temp = '0'
            dateL = nowDate.getFullYear()+"-"+temp+(nowDate.getMonth()+1)+"-"+nowDate.getDate()
        }
        setDate(dateL)
        handleChangeTime(time, dateL)
    }
    async function handleSubmit() {
        ...
        if (taskName === "") {
            ...
            try {
                ...
                respDate.setHours(Number(time.substring(0,2)), Number(time.substring(3,5)))
                let temp = ''
                if(respDate < new Date())
                    respDate = new Date()
                ...
                const response = await axios.post("http://localhost:5000/create-task", {
                    name: taskName,
                    group: selectedGroup,
                    date: respDate.toJSON(),
                    localDay: respDate.getFullYear()+"-"+temp+(respDate.getMonth()+1)+"-"+respDate.getDate(),
                    localTime: respDate.toLocaleTimeString().substring(0,5),
                    priority: priority,
                ...
                if (response.status == 200) {
                    alert("Success S S S S")
                } else {
                    alert("Failed creating task" + response.statusText)
                }
            }
            catch (e){
                alert("Failed creating task")
            }
        }
    }
        ...
        <>
            <div className={styles.container}>
                <div className={styles.header}>
                    <button className={styles.headerButton} onClick={() => {navigate('/home')}}>
                        <svg width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path className={styles.svgArrow}
                ...
                <div className={styles.body}>
                    ...
                    <div className={styles.block}>
                        ...
                        <div className={styles.categories}>
                            ...
                            <select style={selectedGroup === null ? {color: 'rgba(0, 0, 0, 0.2)'} : {}}
                        ...
                        <div className={styles.priorityTable}>
                            <button onClick={() => {
                                setPriority(0)
                                    ...
                                    style={priority === 0 ? {backgroundColor: "#9747ff", color: '#fff'} : {}}>
                                Low
                            </button>
                            <button onClick={() => {
                                setPriority(1)
